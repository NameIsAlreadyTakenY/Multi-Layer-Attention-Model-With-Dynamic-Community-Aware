...
class TILES(object):
    ...
    def __init__(self, data_df=None, g=None, ttl=float('inf'), obs=7, path="", start=0, end=0):
        """
            Constructor
        ...
        self.nodes_com_d=[]
        self.com_nodes_d=[]
        # self.nodes_communities_dict=defaultdict(lambda: [])
        # self.communities_nodes_dict=defaultdict(lambda: [])
    def execute(self):
        """
        ...
        for i in range(len(self.data)):
            ...
            e['weight'] = 1  #这里用来标记边为几重边
            e["u"] = u
            e["v"] = v
            e["time"] = dt # 这里权重可以根据Hawkes process公式计算权重
            ...
            if self.ttl != float('inf'):#float('inf')表示正无穷
    ...
    def remove(self, actual_time, qr):
        ...
        if not qr.empty():
            ...
            if delta < self.ttl:#如果优先队列中最前的边仍然在ttl内，将边再次入队列
                qr.put((timestamp, t[1]))

            else:
                while self.ttl <= delta:
