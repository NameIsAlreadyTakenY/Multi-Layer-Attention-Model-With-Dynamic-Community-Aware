...
class Layer(object):
    ...
    def __call__(self, inputs):
        with tf.name_scope(self.name):
            ...
            outputs = self._call(inputs) #进入类自己的_call中
            if self.logging:
...
class StructuralAttentionLayer(Layer):
    ...
    def sp_attn_head(self, seq, in_sz, out_sz, adj_mat, activation, in_drop=0.0, coef_drop=0.0, residual=False,
        ...
        with tf.variable_scope('struct_attn', reuse=reuse_scope):#reuse：可以是True、None或tf.AUTO_REUSE；如果是True，则我们进入此范围的重用模式以及所有子范围；如果是tf.AUTO_REUSE，则我们创建变量（如果它们不存在），否则返回它们；如果是None，则我们继承父范围的重用标志
            if sparse_inputs:
                weight_var = tf.get_variable("layer_" + str(layer_str) + "_weight_transform", shape=[in_sz, out_sz],dtype=tf.float32) # N*8
                seq_fts = tf.expand_dims(tf.sparse_tensor_dense_matmul(seq, weight_var), axis=0)  # [N, F] 1*N*8
            else:
            ...
            logits = tf.sparse_add(adj_mat * f_1, adj_mat * tf.transpose(f_2))  # adj_mat is [N, N] (sparse) tf.transpose转置张量
