...
class TILES(object):
    ...
    def execute(self):
        ...
        for l in f:
            ...
            if self.ttl != float('inf'):
                ...
                self.remove(dt, qr) # 判断是否有边时间超过ttl，需要移除
    ...
    def remove(self, actual_time, qr):
        ...
        if not qr.empty():
            ...
            if displacement < self.ttl: #没有边需要移除，出队列后入队
                qr.put((timestamp, t[1]))

            else:
                while self.ttl <= displacement: #移除所有ttl时间外的边
    ...
    def add_to_community(self, node, cid): #  {cid:[{node:none}]}
